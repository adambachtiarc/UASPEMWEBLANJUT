...
@section('content')
<p class="h1">Edit Buku</p>
<form id="form-edit-buku" action="{{ url('manajemen-buku/'.$buku->id) }}" method="POST">
    @csrf
    ...
    <div class="mb-3">
        ...
        <textarea name="resume" id="resume" class="form-control" cols="30" rows="10">{{ $buku->resume }}</textarea>
...
<script>
    $(function(){
        ...
        // submit form
        $('#form-edit-buku').on('submit', function (e) {
            e.preventDefault()
            $(".error-text").html("");
            $(".loader").show()
            let url = $("#form-edit-buku").attr('action')
            let method = $("#form-edit-buku").attr('method')
            let formData = new FormData($("#form-edit-buku")[0])
